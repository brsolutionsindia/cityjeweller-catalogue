...
export default function Page() {
  ...
  useEffect(() => {
    const run = async () => {
      setBusy(true);
      try {
        const obj = await getPendingYellowSapphireQueue();
        const arr = Object.values(obj || {}) as QueueItem[];
        // sort by updatedAt/createdAt if present
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-2xl font-bold mt-1">Yellow Sapphire Approvals</div>
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {filtered.map((it) => (
            <Link
              key={it.skuId}
              href={`/admin/yellow-sapphires/${encodeURIComponent(it.skuId)}?gst=${encodeURIComponent(it.gstNumber)}`}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-gray-600 mt-1">GST: <b>{it.gstNumber}</b></div>
                <div className="text-xs text-gray-600">Supplier UID: <b>{it.supplierUid}</b></div>
                <div className="text-xs text-gray-500 mt-2">Status: <b>{it.status}</b></div>
              </div>
            </Link>
          ))}
        </div>
        ...
        {!busy && filtered.length === 0 && (
          <div className="text-sm text-gray-600">No pending yellow sapphire submissions.</div>
        )}
      </div>
    </div>
  );
