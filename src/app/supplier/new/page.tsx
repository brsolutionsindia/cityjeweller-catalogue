...
//  const fnReserve = useMemo(() => httpsCallable(getFunctions(), 'nd_reserveSkuId'), []);
//  const fnCreate  = useMemo(() => httpsCallable(getFunctions(), 'nd_createFromForm'), []);
...
const sizeNum = parseFloat(String(form.Size).trim());
  ...
  if (isNaN(rateUSD) || rateUSD <= 0) {
    ...
    try {
      setSaving(true);
      // 1) reserve SKU with the prefix "8165NDIAMA"
      const r1: any = await fnReserve();
      const skuId = r1.data?.skuId as string;

      const upper = (s: any) => (s == null ? '' : String(s).trim().toUpperCase());

// Map long labels to codes; accept codes too (EX/VG/GD/NA/OT) just in case
const toCode = (v: string) => {
  const t = upper(v);
  if (['EX','VG','GD','NA','OT'].includes(t)) return t;
  if (t.startsWith('EXCELLENT')) return 'EX';
  if (t.startsWith('VERY GOOD')) return 'VG';
  if (t.startsWith('GOOD'))      return 'GD';
  if (t.startsWith('NOT KNOWN') || t === 'NOTKNOWN' || t === 'NA') return 'NA';
  return 'OT'; // Other / anything else
};

// Build normalized fields for the server
const normalized = {
  // rename Symmetry -> Symm and convert Cut/Polish/Symm to codes
  Shape: upper(form.Shape), 
  Cut: toCode(form.Cut),
  Polish: toCode(form.Polish),
  Symm: toCode(form.Symmetry),

  // Uppercase fluorescence; (keeping human-readable in UI)
  Fluorescence: upper(form.Fluorescence),

  // You don't have Status in the form, server will default to 'AVAILABLE'
  // but if you ever add it here, do: Status: upper(form.Status)
};

// 2) server create (writes GST node copy + public record with MRP/OfferPrice)
      const payload = {
        skuId,
        // client sends only form fields; server will read name/mobile/shop internally again if needed
        ...form,
    ...
    <main className="p-6 max-w-3xl mx-auto">
      ...
      <form onSubmit={save} className="grid grid-cols-2 gap-4">
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Shape} onChange={e=>onChange('Shape', e.target.value)}>
  ...
  />
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Clarity} onChange={e=>onChange('Clarity', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Color} onChange={e=>onChange('Color', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Cut} onChange={e=>onChange('Cut', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Polish} onChange={e=>onChange('Polish', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Symmetry} onChange={e=>onChange('Symmetry', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Fluorescence} onChange={e=>onChange('Fluorescence', e.target.value)}>
        ...
        <label className="col-span-1">
          ...
          <select className="border p-2 rounded w-full" value={form.Certified} onChange={e=>onChange('Certified', e.target.value)}>
  ...
  />
        ...
        <label className="col-span-2">
          ...
          <textarea className="border p-2 rounded w-full" value={form.Remarks} onChange={e=>onChange('Remarks', e.target.value)} />
        </label>

        {error && <p className="text-red-600 col-span-2">{error}</p>}
        <div className="col-span-2 flex gap-2 justify-end">
          <button className="border px-3 py-1 rounded" type="button" onClick={()=>router.back()}>Cancel</button>
          <button className="border px-3 py-1 rounded" disabled={saving}>{saving ? 'Saving...' : 'Save'}</button>
        </div>
      </form>
    </main>
  );
